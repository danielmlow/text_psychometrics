...
if location == 'openmind':
	...
	input_dir = '/Users/danielmlow/Dropbox (MIT)/datum/zero_shot/data/input/ctl/'
	output_dir = './data/output/'


set_name = 'X_test_content_validity_prototypicality-3'	


test = pd.read_csv(input_dir + 'X_test_content_validity_prototypicality-3.csv')
...
prompts ={ 
	...
	'google/gemma-2b-it': 
"""
You are a token classification assistant for {context}. 

Here is the token (begins and ends with ```):
```
{document}
```

Classify the following token as to whether it is a prototypical example of {construct} by assigning probabilities for the following labels and return using this JSON format (do not provide additional notes, explanations, or warnings). Provide your best guess, only return JSON:

JSON:
{{'Token is a prototypical example of {construct}': <your_probability>, 'Token is not a prototypical example of {construct}': <your_probability>}}
...
# Classify the following token if it is related to {construct} by assigning probabilities for the following labels and return using this JSON format (do not provide additional notes, explanations, or warnings). Provide your best guess, only return JSON:

# JSON:
# {{'Token is related to {construct}': <your_probability>, 'Token is not related to {construct}': <your_probability>}}
...
#     print = custom_print
	...
	if content_validity:
		documents = test[test['y_test']==dv]['token'].values
		y_test = [1]*len(documents)
	else:
	...
	y_pred_df.to_csv(output_dir_i_dv+f'y_proba_{dv}_content_validity-{content_validity}.csv')
	...
	results = custom_classification_report(y_test, y_pred, y_pred_proba_1, output_dir_i_dv,gridsearch=None,
		best_params=None,feature_vector=None,model_name=f'{model_name_clean}_{dv}_content-validity-3',round_to = 2, ts =ts, save_results=True, dv = dv_clean)
